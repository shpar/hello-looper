...
HelloLooperAudioProcessorEditor::HelloLooperAudioProcessorEditor(HelloLooperAudioProcessor& p)
    : AudioProcessorEditor(&p),
      ...
      processor(p),
      tempoAttachment(p.state, "tempo", tempoSlider),
    ...
    for (int i = 0; i < 4; i++) {
        if (hotkeys[i] != nullptr) {
            (hotkeys[i])->setBounds(10 + i * (getWidth() - 20) / 4, 130, (getWidth() - 20) / 4, 20);
        }
    }
...
void HelloLooperAudioProcessorEditor::paintIfNoFileLoaded(Graphics& g,
...
void HelloLooperAudioProcessorEditor::paintIfFileLoaded(Graphics& g,
                                                        const Rectangle<int>& thumbnailBounds) {
    g.setColour(Colours::black);
    g.fillRect(thumbnailBounds);
    g.setColour(Colours::white);
    thumbnail.drawChannels(g, thumbnailBounds, 0.0, thumbnail.getTotalLength(), 1.0f);
    auto audioPositionPercent =
        (processor.positionSamples / sampleDuration / processor.currentSampleRate);
    auto audioLoopLengthPercent =
        (processor.samplesPerBeat / sampleDuration / processor.currentSampleRate);
    auto drawPosition(audioPositionPercent * thumbnailBounds.getWidth() + thumbnailBounds.getX());
    ...
    if (!chord_analyzer.key_ranges.empty()) {
        ...
        int percent_of_song_length =
            sampleDuration * processor.currentSampleRate / chord_analyzer.brackets_for_analysis;
        ...
        for (const auto& entry : chord_analyzer.key_ranges) {
            ...
            auto keyPositionPercent =
                (entry.first.first / sampleDuration / processor.currentSampleRate);
            auto keyPositionLength = ((entry.first.second - entry.first.first) / sampleDuration /
                                      processor.currentSampleRate);
            Rectangle<int> keyRectangle(
                std::round(keyPositionPercent * thumbnailBounds.getWidth()) +
    ...
    Rectangle<int> thumbnailLoopRect(drawPosition, thumbnailBounds.getY(),
                                     audioLoopLengthPercent * thumbnailBounds.getWidth(),
                                     thumbnailBounds.getHeight());
...
void HelloLooperAudioProcessorEditor::resized() {
    ...
    clearButton.setBounds((mainComponentWidth - 20) / 2 + 10, 10, (mainComponentWidth - 20) / 2,
                          20);
    ...
    syncBeatButton.setBounds((mainComponentWidth - 20) / 4 + 10, 160, (mainComponentWidth - 20) / 4,
                             20);
    exportButton.setBounds((mainComponentWidth - 20) * 2 / 4 + 10, 160,
                           (mainComponentWidth - 20) / 4, 20);
    analyzeButton.setBounds((mainComponentWidth - 20) * 3 / 4 + 10, 160,
                            (mainComponentWidth - 20) / 4, 20);
}
...
void HelloLooperAudioProcessorEditor::checkForBuffersToFree() {
    for (int i = processor.buffers.size(); --i >= 0;) {
        ...
        if (buffer->getReferenceCount() == 2) processor.buffers.remove(i);
    }
}
...
void HelloLooperAudioProcessorEditor::updatePosition() {
    processor.positionSamples =
        positionSlider.getValue() * processor.currentSampleRate * sampleDuration;
    repaint();
}
...
void HelloLooperAudioProcessorEditor::updateTempo(
    AudioPlayHead::CurrentPositionInfo currentInfoFromHost) {
    ...
    if (currentInfoFromHost.isPlaying && syncBeatButton.getToggleState()) {
        ...
        processor.syncOffsetSamples =
            processor.samplesPerBeat - timeInSamples % static_cast<int64>(processor.samplesPerBeat);
...
void HelloLooperAudioProcessorEditor::exportButtonClicked() {
    FileChooser chooser("Save loop as...", File(), "*.wav");
    if (chooser.browseForFileToSave(true)) {
        ...
        std::unique_ptr<AudioFormatWriter> writer(
            WavAudioFormat().createWriterFor(fos, systemSampleRate, 2, 16, nullptr, 0));
...
void HelloLooperAudioProcessorEditor::analyzeButtonClicked() {
    ...
    for (int i = 0; i < n_bracket_analysis; ++i) {
        chord_analyzer.analyze(processor.currentSampleRate, 2, percent_of_song_length,
                               processor.currentBuffer, percent_of_song_length * i);
        ...
        if (i > 0 && r != previous_key) {
            std::pair<std::pair<int, int>, int> temp{
                {percent_of_song_length * (i - same_key_intervals), percent_of_song_length * i - 1},
                previous_key};
...
void HelloLooperAudioProcessorEditor::positionSliderChanged() {
    if (processor.currentSampleRate > 0.0) updatePosition();
}
